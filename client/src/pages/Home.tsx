 ...
 * Faithful reproduction of the original Glitch Pop neon design
 */
...
const ASSETS = {
  ...
  buttonJouer: "https://files.manuscdn.com/user_upload_by_module/session_file/310519663278017338/qbYQUrlrZLtgpbrA.png",
  targetFire: "https://files.manuscdn.com/user_upload_by_module/session_file/310519663278017338/rhkhKnrsvlrdtdyK.png",
  targetClassic: "https://files.manuscdn.com/user_upload_by_module/session_file/310519663278017338/DBSWOFFTnVIUpPwe.png",
  targetNeon: "https://files.manuscdn.com/user_upload_by_module/session_file/310519663278017338/cGLfLwYwFPjpHeda.png",
  hitFx: "https://files.manuscdn.com/user_upload_by_module/session_file/310519663278017338/cVzjnUevsZAPKZTP.png",
  heroArcade: "https://files.manuscdn.com/user_upload_by_module/session_file/310519663278017338/EepYpRIAAUNPttlb.jpeg",
  uiPattern: "https://files.manuscdn.com/user_upload_by_module/session_file/310519663278017338/kbJNxmeJGXSVTVEd.jpeg",
  logoSkull: "https://files.manuscdn.com/user_upload_by_module/session_file/310519663278017338/exQiasCismPDLqNH.jpeg",
};
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#050818] text-white overflow-hidden" onMouseMove={handleMouseMove}>
      ...
      {/* === NAV BAR === */}
      <nav className="fixed top-0 left-0 right-0 z-50 px-4 py-3" style={{ background: 'rgba(5,8,24,0.85)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(0,240,255,0.1)' }}>
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center px-4">
        ...
        <div className="absolute inset-0 overflow-hidden">
          ...
          <div className="absolute bottom-0 left-0 right-0 h-[50vh] opacity-20"
            style={{
              backgroundImage: `url(${ASSETS.heroArcade})`,
        ...
        <div className="relative z-10 text-center max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-3 gap-3 sm:gap-6 max-w-lg mx-auto">
            <div className="text-center p-3 rounded-xl" style={{ background: 'rgba(0,240,255,0.05)', border: '1px solid rgba(0,240,255,0.15)' }}>
              <div className="text-2xl sm:text-3xl font-black text-[#00f0ff]" style={{ textShadow: '0 0 20px rgba(0,240,255,0.6)' }}>60</div>
            ...
            <div className="text-center p-3 rounded-xl" style={{ background: 'rgba(255,0,255,0.05)', border: '1px solid rgba(255,0,255,0.15)' }}>
              <div className="text-2xl sm:text-3xl font-black text-[#ff00ff]" style={{ textShadow: '0 0 20px rgba(255,0,255,0.6)' }}>4</div>
            ...
            <div className="text-center p-3 rounded-xl" style={{ background: 'rgba(255,255,0,0.05)', border: '1px solid rgba(255,255,0,0.15)' }}>
              <div className="text-2xl sm:text-3xl font-black text-[#ffff00]" style={{ textShadow: '0 0 20px rgba(255,255,0,0.6)' }}>{highScore > 0 ? highScore : '---'}</div>
      ...
      <section className="relative py-16 sm:py-24 px-4">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 sm:gap-8 mb-16 max-w-3xl mx-auto">
            {[
              { src: ASSETS.logo, label: 'PLAYER', color: '#00f0ff', glow: 'rgba(0,240,255,0.5)' },
              { src: ASSETS.targetFire, label: 'FIRE TARGET', color: '#ff6600', glow: 'rgba(255,102,0,0.5)' },
              { src: ASSETS.targetClassic, label: 'CLASSIC TARGET', color: '#0066ff', glow: 'rgba(0,100,255,0.5)' },
              { src: ASSETS.targetNeon, label: 'POWER-UP', color: '#ff00ff', glow: 'rgba(255,0,255,0.5)' },
            ].map((item) => (
              <div key={item.label} className="text-center group">
                <div className="relative w-20 h-20 sm:w-24 sm:h-24 mx-auto mb-3 transition-transform group-hover:scale-110">
                  <img src={item.src} alt={item.label} className="w-full h-full" style={{ filter: `drop-shadow(0 0 15px ${item.glow})` }} />
                </div>
                <p className="text-[10px] sm:text-xs font-mono tracking-wider" style={{ color: item.color }}>{item.label}</p>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {[
              { title: 'NEON RENDERER', desc: 'Custom canvas engine with dynamic glow, chromatic aberration, scanlines and shockwave effects at 60 FPS.', color: '#00f0ff', icon: 'M13 10V3L4 14h7v7l9-11h-7z' },
              { title: 'PROCEDURAL AUDIO', desc: 'Real-time synthwave soundtrack generated via Web Audio API. No loading, no buffering, pure vibes.', color: '#ff00ff', icon: 'M9 18V5l12-2v13M9 18c0 1.66-1.34 3-3 3s-3-1.34-3-3 1.34-3 3-3 3 1.34 3 3zM21 16c0 1.66-1.34 3-3 3s-3-1.34-3-3 1.34-3 3-3 3 1.34 3 3z' },
              { title: 'COMBO SYSTEM', desc: '7 combo tiers from Good to Godlike. Near-miss detection rewards precision with massive score multipliers.', color: '#ffff00', icon: 'M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z' },
              { title: 'POWER-UPS', desc: 'Shield, SlowMo, Magnet, Multiplier, Bomb and more. Each with unique visual effects and strategic value.', color: '#00ff88', icon: 'M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z' },
              { title: 'ADAPTIVE DIFFICULTY', desc: 'AI-driven difficulty that keeps you in the flow state. From Easy to Nightmare, the game adapts to YOU.', color: '#ff6600', icon: 'M2 20h20L12 4 2 20zm11-3h-2v-2h2v2zm0-4h-2V9h2v4z' },
              { title: 'PARTICLE SYSTEM', desc: 'Up to 1000 simultaneous particles. Explosions, trails, sparkles and confetti for every action.', color: '#ff2d7b', icon: 'M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707' },
            ].map((feature) => (
              <div
                key={feature.title}
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-10 h-10 rounded-lg flex items-center justify-center" style={{ background: `${feature.color}15`, border: `1px solid ${feature.color}30` }}>
                  ...
                  <h3 className="font-bold text-sm sm:text-base tracking-wider" style={{ color: feature.color }}>{feature.title}</h3>
                </div>
            ...
            {[
              ...
              <Link key={mode.name} href="/game">
                <div
                  ...
                  style={{ background: `linear-gradient(135deg, ${mode.color}08 0%, #050818 100%)`, borderColor: `${mode.color}30` }}
                  onMouseEnter={(e) => {
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="text-xl sm:text-2xl font-black tracking-wider" style={{ color: mode.color }}>{mode.name}</h3>
                    <span className="text-[9px] sm:text-[10px] font-bold px-2 py-1 rounded-full tracking-wider" style={{ background: `${mode.color}15`, color: mode.color, border: `1px solid ${mode.color}30` }}>{mode.tag}</span>
                  </div>
                  <p className="text-gray-400 text-xs sm:text-sm leading-relaxed mb-4">{mode.desc}</p>
                  <div className="flex items-center gap-2 text-xs font-bold tracking-wider opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: mode.color }}>
                    <span>PLAY NOW</span>
      ...
      <section className="relative py-16 sm:py-24 px-4">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-center text-3xl sm:text-4xl md:text-5xl font-black mb-4 tracking-tight">
            ...
            <span className="text-white ml-3">PASS</span>
          </h2>
          <p className="text-center text-gray-500 text-sm sm:text-base mb-12 max-w-xl mx-auto">
            Unlock the full Fail Frenzy experience. No ads, unlimited modes, exclusive skins, and global leaderboards.
          </p>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-3xl mx-auto">
            <div className="p-6 rounded-xl" style={{ background: 'rgba(13,18,48,0.6)', border: '1px solid rgba(0,240,255,0.15)' }}>
              <h3 className="text-xl font-black mb-1 text-white">FREE</h3>
              <p className="text-gray-500 text-xs font-mono mb-6">BASIC ACCESS</p>
              <div className="space-y-3 mb-6">
                {['Classic Mode', '3 Basic Skins', 'Local Leaderboard', 'Ads Between Games'].map((f) => (
                  <div key={f} className="flex items-center gap-2 text-sm text-gray-400">
                    <span style={{ color: '#00f0ff' }}>&#10003;</span> {f}
                  </div>
                ))}
                {['All Game Modes', 'Premium Skins', 'Global Leaderboard', 'Daily Challenges'].map((f) => (
              ...
              <Link href="/game">
                <button className="w-full py-3 rounded-lg text-sm font-bold tracking-wider transition-all hover:scale-[1.02]" style={{ background: 'rgba(0,240,255,0.1)', border: '1px solid rgba(0,240,255,0.3)', color: '#00f0ff' }}>PLAY FREE</button>
              </Link>
            </div>

            <div className="p-6 rounded-xl relative overflow-hidden" style={{ background: 'linear-gradient(135deg, rgba(255,0,255,0.08) 0%, rgba(0,240,255,0.08) 100%)', border: '2px solid rgba(255,0,255,0.4)' }}>
              <div className="absolute top-0 right-0 px-3 py-1 text-[10px] font-bold tracking-wider rounded-bl-lg" style={{ background: '#ff00ff', color: '#050818' }}>BEST VALUE</div>
              <h3 className="text-xl font-black mb-1" style={{ color: '#ff00ff' }}>PREMIUM</h3>
              <div className="flex items-baseline gap-1 mb-6">
                <span className="text-3xl font-black text-white">4.99</span>
                <span className="text-gray-400 text-sm">&euro;/mois</span>
              </div>
              <div className="space-y-3 mb-6">
                {['All 4 Game Modes', '20+ Premium Skins', 'Global Leaderboard', 'Daily Challenges', 'No Ads Ever', 'Advanced Stats', 'Priority Support', 'Exclusive Events'].map((f) => (
              ...
              <Link href="/premium">
                <button className="w-full py-3 rounded-lg text-sm font-bold tracking-wider transition-all hover:scale-[1.02]" style={{ background: 'linear-gradient(90deg, #ff00ff, #00f0ff)', color: '#050818' }}>GO PREMIUM</button>
              </Link>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-3xl mx-auto text-center">
          ...
          <div className="flex items-center justify-center gap-8 sm:gap-12 mb-8">
            <div className="text-center">
              <img src={ASSETS.hitFx} alt="Hit Effect" className="w-24 h-24 sm:w-32 sm:h-32 mx-auto mb-3"
            ...
            <div className="text-center">
              <img src={ASSETS.targetNeon} alt="Neon Glow" className="w-24 h-24 sm:w-32 sm:h-32 mx-auto mb-3"
      ...
      <section className="relative py-20 sm:py-32 px-4 text-center">
        <div className="absolute inset-0 overflow-hidden">
          <div className="absolute inset-0 opacity-15" style={{ backgroundImage: `url(${ASSETS.uiPattern})`, backgroundSize: 'cover', backgroundPosition: 'center', filter: 'blur(20px) saturate(1.5)' }} />
          <div className="absolute inset-0 bg-[#050818]/85" />
        </div>

        <div className="relative z-10 max-w-3xl mx-auto">
          <img src={ASSETS.logoSkull} alt="Skull" className="w-20 h-20 sm:w-24 sm:h-24 mx-auto mb-6 rounded-full"
          ...
          <h2 className="text-4xl sm:text-5xl md:text-6xl font-black mb-6 tracking-tight leading-tight">
            ...
            <span style={{ background: 'linear-gradient(90deg, #00f0ff, #ff00ff, #ffff00)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>FAIL FORWARD?</span>
          </h2>
          ...
          <Link href="/game">
            <button className="group relative transition-all duration-300 hover:scale-110 active:scale-95">
              <img src={ASSETS.buttonJouer} alt="JOUER" className="w-52 sm:w-60 md:w-72 mx-auto"
                style={{ filter: 'drop-shadow(0 0 30px rgba(0,240,255,0.4)) drop-shadow(0 0 60px rgba(255,0,255,0.2))' }} />
            </button>
          </Link>
        </div>
      </section>
      ...
      <footer className="border-t py-8 px-4" style={{ borderColor: 'rgba(0,240,255,0.1)' }}>
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <img src={ASSETS.logo} alt="FF" className="w-7 h-7" style={{ filter: 'drop-shadow(0 0 5px rgba(0,240,255,0.4))' }} />
