...
const LoadingScreen: React.FC<{ progress: number }> = ({ progress }) => {
  ...
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-center z-50 rounded-xl" style={{ background: '#050818' }}>
      <div className="mb-8">
        <img src={`${BASE}logo-skull-imposing.png`} alt="" className="w-24 h-24 sm:w-32 sm:h-32 mx-auto rounded-full object-cover" style={{ filter: 'drop-shadow(0 0 30px rgba(0,240,255,0.6))', boxShadow: '0 0 25px rgba(0,240,255,0.4), 0 0 50px rgba(255,0,255,0.2)', animation: 'ffSpin 3s linear infinite' }} />
      </div>
...
export const GameCanvas: React.FC<GameCanvasProps> = ({ mode, assets, onScoreUpdate, onGameOver }) => {
  ...
  useEffect(() => {
    ...
    game.start();
    // Pause immédiatement si l'overlay d'instructions est affiché
    game.pause();
  ...
  return (
    <div className="game-canvas-container relative w-full max-w-[900px] mx-auto">
      ...
      {isPaused && !showGameOver && (
        <div className="absolute inset-0 flex items-center justify-center rounded-xl z-20" style={{ background: 'rgba(5,8,24,0.92)', backdropFilter: 'blur(8px)' }}>
          <div className="text-center px-6">
            <img src={`${BASE}logo-skull-imposing.png`} alt="" className="w-16 h-16 mx-auto mb-4 rounded-full object-cover opacity-60" style={{ filter: 'drop-shadow(0 0 20px rgba(0,240,255,0.4))' }} />
      ...
      {showInstructions && !showGameOver && (
        <div
          className="absolute inset-0 flex items-center justify-center rounded-xl z-30 cursor-pointer"
          style={{ background: 'rgba(5,8,24,0.92)', backdropFilter: 'blur(8px)' }}
          onClick={() => { setShowInstructions(false); if (gameRef.current) gameRef.current.resume(); }}
          onTouchStart={() => { setShowInstructions(false); if (gameRef.current) gameRef.current.resume(); }}
        >
          <div className="text-center px-6 max-w-md">
            <img src={`${BASE}logo-skull-imposing.png`} alt="" className="w-20 h-20 mx-auto mb-6 rounded-full object-cover" style={{ filter: 'drop-shadow(0 0 30px rgba(0,240,255,0.6))', animation: 'ffSpin 3s linear infinite' }} />
            <h2 className="text-3xl sm:text-4xl font-black mb-6" style={{ color: '#00f0ff', textShadow: '0 0 40px rgba(0,240,255,0.6)' }}>COMMENT JOUER</h2>
            
            <div className="space-y-4 mb-8">
              {/* Mobile */}
              <div className="p-4 rounded-xl" style={{ background: 'rgba(0,240,255,0.08)', border: '1px solid rgba(0,240,255,0.3)' }}>
                <div className="text-2xl mb-2">{'📱'}</div>
                <p className="text-sm font-bold" style={{ color: '#00f0ff' }}>MOBILE / TACTILE</p>
                <p className="text-xs text-gray-400 mt-1">Touchez et glissez pour déplacer le joueur</p>
              </div>
              {/* Desktop */}
              <div className="p-4 rounded-xl" style={{ background: 'rgba(255,0,255,0.08)', border: '1px solid rgba(255,0,255,0.3)' }}>
                <div className="text-2xl mb-2">{'🖥️'}</div>
                <p className="text-sm font-bold" style={{ color: '#ff00ff' }}>PC / CLAVIER</p>
                <p className="text-xs text-gray-400 mt-1">Flèches directionnelles ou WASD pour bouger</p>
              </div>
              {/* Objectif */}
              <div className="p-4 rounded-xl" style={{ background: 'rgba(255,215,0,0.08)', border: '1px solid rgba(255,215,0,0.3)' }}>
                <div className="text-2xl mb-2">{'🎯'}</div>
                <p className="text-sm font-bold" style={{ color: '#ffd700' }}>OBJECTIF</p>
                <p className="text-xs text-gray-400 mt-1">Évitez les obstacles, collectez les power-ups, survivez !</p>
              </div>
            </div>

            <div className="animate-pulse">
              <p className="text-lg font-black tracking-wider" style={{ color: '#00ff88', textShadow: '0 0 20px rgba(0,255,136,0.5)' }}>CLIQUEZ POUR COMMENCER</p>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="max-w-[900px] mx-auto flex items-center justify-between">
          <Link href="/">
            <div className="flex items-center gap-2.5 cursor-pointer group">
              <img src={`${BASE}logo-skull-imposing.png`} alt="" className="w-8 h-8 sm:w-9 sm:h-9 rounded-full object-cover transition-transform group-hover:rotate-12" style={{ filter: 'drop-shadow(0 0 8px rgba(0,240,255,0.5))', boxShadow: '0 0 12px rgba(0,240,255,0.3)' }} />
        ...
        {!selectedMode ? (
          <div className="max-w-2xl mx-auto">
            <div className="text-center mb-8 sm:mb-10">
              <img src={`${BASE}logo-skull-imposing.png`} alt="" className="w-24 h-24 sm:w-32 sm:h-32 mx-auto mb-4 rounded-full object-cover" style={{ filter: 'drop-shadow(0 0 25px rgba(0,240,255,0.5))', boxShadow: '0 0 30px rgba(0,240,255,0.3)' }} />
